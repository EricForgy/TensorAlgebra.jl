...
function Covector(vs::VectorSpace{K,L,D}, a) where {K,L,D}
    size(a) === D || throw(DimensionMismatch("Tried to create a covector of dimension $(size(a)) in the vector space $(label(vs)) of dimension $(D)."))
    Tensor{K,1,TensorSpace{K,1,(VectorSpace{K,L,D}(),)}}(a)
end

function Vector(vs::VectorSpace{K,L,D}, a) where {K,L,D}
    size(a) === D || throw(DimensionMismatch("Tried to create a vector of dimension $(size(a)) in the vector space $(label(vs)) of dimension $(D)."))
    Tensor{K,1,TensorSpace{K,1,(DualSpace{K,L,D}(),)}}(a)
end

VectorSpace(L::Symbol,::Type{K}; dims::NTuple{D,Int}) where {K,D} = VectorSpace{K,L,dims}()
